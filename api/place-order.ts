...
// Avoid importing `@vercel/node` to prevent "Cannot find module '@vercel/node'" errors
// in environments where the package is not installed. Use `any` for handler types.
...
export default async function handler(req: any, res: any) {
  ...
  try {
    const { shippingInfo, cartItems, subtotal, tax, total } = req.body ?? {};
    ...
    const rows = (cartItems as CartItem[])
      .map((item) => {
    ...
    const html = `
      <div style="font-family:Arial,Helvetica,sans-serif;color:#111">
        ...
        <div style="max-width:720px;margin-top:16px;padding:12px;border:1px solid #f0f0f0;background:#fafafa">
          <p style="margin:6px 0"><strong>Tạm tính:</strong> ${Number(subtotal).toFixed(3)}đ</p>
          <p style="margin:6px 0;font-size:1.1em"><strong>Tổng cộng:</strong> ${Number(total).toFixed(3)}đ</p>
        </div>
    ...
    const mailOptions = {
      ...
      subject: `Đon hàng mới từ ${customerName} — ${Number(total).toFixed(3)}đ`,
      html,
